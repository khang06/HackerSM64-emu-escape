...
ALL_DIRS := $(BUILD_DIR) $(addprefix $(BUILD_DIR)/,$(SRC_DIRS) src/native asm/debug $(GODDARD_SRC_DIRS) $(LIBZ_SRC_DIRS) $(ULTRA_BIN_DIRS) $(BIN_DIRS) $(TEXTURE_DIRS) $(TEXT_DIRS) $(SOUND_SAMPLE_DIRS) $(addprefix levels/,$(LEVEL_DIRS)) rsp include) $(YAY0_DIR) $(addprefix $(YAY0_DIR)/,$(VERSION)) $(SOUND_BIN_DIR) $(SOUND_BIN_DIR)/sequences/$(VERSION)
...
# Emulator escape code
$(BUILD_DIR)/src/native/shellcode: src/native/shellcode.asm
	$(call print,Assembling (NASM):,$<,$@)
	$(V)nasm -f bin $< -o $@

$(BUILD_DIR)/src/native/shellcode_swap: $(BUILD_DIR)/src/native/shellcode
	$(call print,Byteswapping:,$<,$@)
	$(V)python3 tools/byteswap.py $< $@

$(BUILD_DIR)/src/boot/emulator_escape.o: $(BUILD_DIR)/src/native/shellcode_swap.inc.c
